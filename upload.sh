...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' YZJA.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" YZJA.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' YZJA.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" YZJA.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' YZJA.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" YZJA.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' YZJA.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' YZJA.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" YZJA.podspec
...
pod repo push ${Repo} YZJA.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
